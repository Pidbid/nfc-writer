...
const { tag, busy, connectedReader, notice, error, readTag } = useNfc();
const message = useMessage();

watch(notice, (v) => { if (v) message.success(v); });
watch(error, (v) => { if (v) message.error(v); });

const hasRecords = computed(() => (tag.value?.records.length ?? 0) > 0);
</script>
...
<template>
  <div class="read-view">
    ...
    <div class="action-bar fade-up">
      ...
      <span v-if="!connectedReader" class="warning-hint">
        请先在仪表盘连接读写器
      </span>
    </div>
    ...
    <Transition name="slide-fade" mode="out-in">
      <div v-if="tag" key="has-tag" class="tag-content">
        <!-- 标签元信息 -->
        <NCard class="meta-card fade-up" :style="{ animationDelay: '80ms' }">
          <div class="meta-grid">
            <div class="meta-item">
              <span class="meta-label">UID</span>
              <code class="meta-value uid">{{ tag.uid }}</code>
            </div>
            <div class="meta-item">
              <span class="meta-label">记录数</span>
              <span class="meta-value">{{ tag.records.length }}</span>
            </div>
            <div class="meta-item">
        ...
        <!-- NDEF 记录列表 -->
        <NCard title="NDEF 记录" class="records-card fade-up" :style="{ animationDelay: '160ms' }">
          <template #header-extra>
            <NIcon :size="18" color="#8a919f"><DocumentTextOutline /></NIcon>
          </template>

          <TransitionGroup name="list" tag="div" class="records-list">
            <div
              v-for="(record, index) in tag.records"
              :key="index"
              class="record-block"
            >
              ...
              <div class="record-body">
                <pre class="record-content">{{ record }}</pre>
              </div>
            </div>
          </TransitionGroup>
      ...
      <div v-else key="no-tag" class="empty-state fade-up">
        <div class="scan-animation">
          ...
          <div class="scan-icon">
            <NIcon :size="40" color="var(--color-primary)"><PhonePortraitOutline /></NIcon>
          </div>
        </div>
...
/* ── 动画 ── */
.fade-up {
  animation: fade-up-in 0.5s cubic-bezier(0.4, 0, 0.2, 1) both;
}
