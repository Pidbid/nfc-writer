...
import type { HistoryEntry } from "../composables/useNfc";

const { history } = useNfc();

const filter = ref<"all" | HistoryEntry["action"]>("all");

const filtered = computed(() => {
  if (filter.value === "all") return history.value;
  return history.value.filter((e) => e.action === filter.value);
});
...
> = {
  read: { label: "读取", icon: ReaderOutline, color: "#2080f0" },
  write: { label: "写入", icon: CreateOutline, color: "#18a058" },
  connect: { label: "连接", icon: LinkOutline, color: "#8a5cf5" },
  disconnect: { label: "断开", icon: LogOutOutline, color: "#f0a020" },
};
...
<template>
  <div class="history-view">
    ...
    <!-- 日志列表 -->
    <NCard class="log-card fade-up" :style="{ animationDelay: '100ms' }">
      <template #header-extra>
        <NIcon :size="18" color="#8a919f"><TimeOutline /></NIcon>
      </template>
        ...
        >
          ...
          <div class="log-body">
            <div class="log-top">
              ...
              <span class="log-time">
                <NTime :time="entry.time" format="HH:mm:ss" />
              </span>
            </div>
